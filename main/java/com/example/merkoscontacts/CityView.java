...
public class CityView extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        temp.addProperty("city", city);
        query = temp.toString();
        this.setTitle(city);
        if (intent.getStringExtra("EXACT") != null) {
            exact = intent.getStringExtra("EXACT");
        }
            ...
            public void onResponse(Call call, final Response response) {
                try (final ResponseBody responseBody = response.body()) {
                    ...
                    if (!response.isSuccessful())
                        ...
                        CityView.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                ...
                                NavigationView.OnNavigationItemSelectedListener messageClickedHandler = new NavigationView.OnNavigationItemSelectedListener() {
                                    ...
                                    public boolean onNavigationItemSelected(@NonNull MenuItem menuItem) {
                                        if (menuItem.toString().equals("Advanced Search")) {
                                            ...
                                            sendEmail();
                                            onBackPressed();
                                        } else if (menuItem.toString().equals("City Comments")){
                                            viewComments();
                                        } else {
    ...
    public void getContacts() {
        ...
        Request requestContacts = new Request.Builder()
                ...
                .header("Authorization", token_type + " " + token)
                .post(RequestBody.create(empty, MEDIA_TYPE_JSON))
                .build();
        ...
        public void onBindChildViewHolder(@NonNull ChildViewHolder childViewHolder, int parentPosition, int childPosition, @NonNull Object C) {
            ...
            if (child.phone != null) {
                ...
                for (int i = 0; i < child.phone.length; i++) {
                    if (i != child.phone.length -1)
                        phones = phones + child.phone[i].toString() + ", ";
                    else
                        phones = phones + child.phone[i].toString();
                }
                phones = "Phone: " + phones;
            ...
            contactsChildViewHolder.updateButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    CityView.this.Update(child._id, true);
                    onBackPressed();
                }
            });
        }
